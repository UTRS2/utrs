            ...
            @foreach($appealmap as $appeal)
              ...
              @endif
                ...
                <div class="tracking-content">{{$appeal['text']}}<span>{{$appeal['time']}}</span>
                  ...
                  @auth
                      @if($isdev)
                        <a href="/appeal/{{$appeal['appealid']}}"><button type="submit" class="btn btn-primary">{{__('appeals.map.reviewappeal')}}</button></a>
                      @endif
                  @endauth
                  @guest
                    ...
                    <button type="submit" class="btn btn-primary">{{__('appeals.map.reviewappeal')}}</button>
                    @endif
                    {{ html()->form()->close() }}
                  @endguest
                  @endif
